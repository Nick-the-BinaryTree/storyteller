...
describe('Four Tree test suite', () => {
    ...
    it('adds one node', () => {
        ...
        expect(t.get(0)).toBe({ ...TEST_NODE, index: 0 });
    });
    ...
    it('adds six nodes', () => {
        ...
        expect(t.get(5)).toBe({ ...TEST_NODE, index: 5 });
    ...
    it('removes three leaves then root', () => {
        ...
        expect(t.get(5)).toBe({ ...TEST_NODE, index: 4 });
        t.remove(4);
        expect(t.get(4)).toBeNull();

        expect(t.get(5)).toBe({ ...TEST_NODE, index: 3 });
        t.remove(3);
    ...
    it('removes branch', () => {
        ...
        expect(t.get(1)).toBe({ ...TEST_NODE, index: 1 });
        expect(t.get(5)).toBe({ ...TEST_NODE, index: 5 });
        expect(t.get(21)).toBe({ ...TEST_NODE, index: 21 });
